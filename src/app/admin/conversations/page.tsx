    ...
    return (
        <div className="p-8 h-[calc(100vh-2rem)] flex flex-col space-y-6 animate-in fade-in zoom-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
                <div>
                    <h1 className="text-3xl font-bold text-black dark:text-white tracking-tight flex items-center gap-3">
                        Inbox Neural
                        <span className="text-sm font-normal text-black/40 dark:text-white/40 px-3 py-1 bg-black/5 dark:bg-white/10 rounded-full border border-black/5 dark:border-white/5">
            ...
            <div className="flex flex-col lg:flex-row gap-6 flex-1 overflow-hidden">
                {/* LIST View */}
                <div className="w-full lg:w-[400px] flex flex-col bg-white/60 dark:bg-black/20 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-3xl overflow-hidden shrink-0 shadow-xl transition-all duration-300">
                    <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
                        {loading ? (
                            <div className="flex items-center justify-center py-20">
                                <div className="w-8 h-8 border-2 border-black/20 dark:border-white/20 border-t-amber-500 rounded-full animate-spin" />
                            </div>
                        ) : sessions.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20 text-center px-6">
                                <div className="w-12 h-12 bg-black/5 dark:bg-white/5 rounded-full flex items-center justify-center mb-4">
                                    <svg className="w-6 h-6 text-black/20 dark:text-white/20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" /></svg>
                                </div>
                                <p className="text-black/40 dark:text-white/40 text-sm font-medium">No se encontraron conversaciones</p>
                            </div>
                        ) : (
                            sessions.map((session) => (
                                <button
                                    ...
                                    className={`w-full text-left p-4 rounded-2xl border transition-all duration-200 group relative overflow-hidden ${selectedSession?.id === session.id
                                        ? 'bg-amber-500/5 dark:bg-red-500/5 border-amber-500/20 dark:border-red-500/20 shadow-md'
                                ...
                                >
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="relative">
                                                {session.classification ? (
                                                    <span
                                                        className={`block w-2.5 h-2.5 rounded-full ${session.classification.flags?.escalation_needed
                                    ...
                                    <div className="pl-5">
                                        {session.classification?.summary ? (
                                            <p className="text-xs text-black/60 dark:text-white/60 line-clamp-2 leading-relaxed mb-3">
                                                {session.classification.summary}
                                            </p>
                                        ) : (
                                            <p className="text-xs text-black/30 dark:text-white/30 italic mb-3">Sin análisis disponible...</p>
                                        )}

                                        <div className="flex gap-2 items-center">
                                            <span className="text-[10px] bg-black/5 dark:bg-white/10 px-2 py-0.5 rounded text-black/50 dark:text-white/50 font-medium">
                ...
                {/* DETAIL View */}
                <div className="flex-1 bg-white/40 dark:bg-white/5 backdrop-blur-2xl border border-black/5 dark:border-white/10 rounded-3xl overflow-hidden flex flex-col shadow-2xl relative transition-all duration-300">
                    {/* Background Aurora */}
                    <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-gradient-to-b from-amber-500/5 to-red-500/5 rounded-full blur-[120px] -mr-32 -mt-32 pointer-events-none" />

                    {selectedSession ? (
                        <>
                            {/* Header Detalle */}
                            <div className="p-6 border-b border-black/5 dark:border-white/5 bg-white/50 dark:bg-white/[0.02] relative z-10 backdrop-blur-md">
                                <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-black to-zinc-800 dark:from-white dark:to-zinc-300 flex items-center justify-center shadow-lg">
                                ...
                                {selectedSession.classification && (
                                    <div className="bg-black/5 dark:bg-black/40 rounded-2xl p-5 border border-black/5 dark:border-white/5 relative overflow-hidden group">
                                        ...
                                        <div className="flex items-start gap-4 relative z-10">
                                            ...
                                            <div className="flex-1">
                                                <h4 className="text-[10px] uppercase tracking-widest font-bold text-black/40 dark:text-white/40 mb-1">Análisis Inteligente</h4>
                                                <p className="text-sm text-black/80 dark:text-white/90 leading-relaxed font-medium">
                                            ...
                                            <div key={i} className={`flex ${isUser ? 'justify-end' : 'justify-start'} group`}>
                                                <div className={`flex flex-col max-w-[85%] ${isUser ? 'items-end' : 'items-start'}`}>
                                                    <div className="flex items-end gap-3">
                                                        {!isUser && (
                                                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-red-600 flex items-center justify-center shrink-0 shadow-lg mb-1">
                                                        ...
                                                        <div
                                                            className={`px-6 py-3.5 rounded-2xl text-sm leading-relaxed shadow-sm backdrop-blur-md border transition-all duration-300 ${isUser
                                                                ? 'bg-black/80 dark:bg-white text-white dark:text-black rounded-tr-sm border-black/10 dark:border-white/20'
                                                                : 'bg-white/80 dark:bg-white/10 text-black dark:text-white rounded-tl-sm border-white/40 dark:border-white/10'
                                                                }`}
                                                        >
                                                            {content}
                                                        </div>
                                                        ...
                                                        {isUser && (
                                                            <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-white/10 flex items-center justify-center shrink-0 mb-1">
                                                                <svg className="w-4 h-4 text-gray-500 dark:text-white/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                                                            </div>
                                                        )}
                                                    </div>
                                                    <span className={`text-[10px] mt-1.5 font-medium opacity-0 group-hover:opacity-100 transition-opacity ${isUser ? 'pr-12 text-black/30 dark:text-white/30' : 'pl-12 text-black/30 dark:text-white/30'}`}>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 relative overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-tr from-amber-500/5 to-red-500/5 backdrop-blur-3xl" />
                            <div className="relative z-10 w-24 h-24 bg-gradient-to-br from-white to-gray-100 dark:from-white/5 dark:to-white/10 rounded-[2rem] flex items-center justify-center mb-8 shadow-2xl border border-white/20 dark:border-white/10 animate-float-bg">
                                <svg className="w-10 h-10 text-amber-500 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                                </svg>
                            </div>
                            <h3 className="text-2xl font-bold text-black dark:text-white mb-2 relative z-10">Selecciona una Conversación</h3>
                            <p className="text-black/40 dark:text-white/40 max-w-sm relative z-10">Explora el historial detallado de interacciones, análisis de sentimiento y métricas de calidad.</p>
                        </div>
                    )}
                </div>
            </div>
